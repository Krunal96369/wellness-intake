...
async function loadConfigForSubmission(sub: SubmissionDoc): Promise<FormConfigShape> {
  ...
  if (!config) {
    config = await FormConfig.findOne({ key: sub.configKey, isActive: true }).lean();
  }
  if (!config) {
    throw new ApiError(500, 'Form configuration for this submission is missing');
  }
...
submissionsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = await Submission.find(filter)
      .select('title status completedSteps totalSteps currentStep createdAt updatedAt')
      .sort({ createdAt: -1 })
      .lean();
...
submissionsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const config = await FormConfig.findOne({ key: configKey, isActive: true }).lean();
    if (!config) throw new ApiError(404, `No active configuration for "${configKey}"`);
...
submissionsRouter.get(
  '/:id',
  asyncHandler(async (req, res) => {
    const sub = await Submission.findOne({ _id: req.params.id, userId: USER_ID });
...
submissionsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const merged: Answers = { ...(sub.answers as Answers), ...(parsed.data.answers ?? {}) };
...
submissionsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (Object.keys(errors).length) {
      throw new ApiError(422, 'Please fix the highlighted fields before submitting', errors);
    }
...
submissionsRouter.delete(
  '/:id',
  asyncHandler(async (req, res) => {
    const result = await Submission.deleteOne({ _id: req.params.id, userId: USER_ID });
    if (result.deletedCount === 0) throw new ApiError(404, 'Submission not found');
    res.status(204).end();
  }),
);
